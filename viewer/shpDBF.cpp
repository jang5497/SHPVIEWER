...
int shpDBF::Open( const char * pszFilename, const char * pszAccess )
{
  ...
  /*      Open the file.                                                  */
  /* -------------------------------------------------------------------- */
  m_fp = fopen( pszDBFFilename, pszAccess );
  if (m_fp == NULL )
    return( FALSE );
...
void* shpDBF::ReadAttribute(int hEntity, int iField, char chReqType)
{
  ...
  /* -------------------------------------------------------------------- */
  strncpy( pszStringField, (const char*)pabyRec+m_panFieldOffset[iField],
																		...
																		int * pnDecimals)
{
  ...
  if (pszFieldName != NULL ) {
    ...
    strncpy( pszFieldName, (char *) m_pszHeader+iField*32, 11 );
    pszFieldName[11] = '\0';
...
int shpDBF::WriteAttribute(int hEntity, int iField, void * pValue )
{
  ...
  case 'N':
    if (m_panFieldDecimals[iField] == 0 ) {
      sprintf( szFormat, "%%%dd", m_panFieldSize[iField] );
      sprintf(szSField, szFormat, (int) *((double *) pValue) );
      if ((int)strlen(szSField) > m_panFieldSize[iField] )
				szSField[m_panFieldSize[iField]] = '\0';
      strncpy((char *) (pabyRec+m_panFieldOffset[iField]), szSField, strlen(szSField) );
    }
    else {
  ...
  default:
    ...
    strncpy((char *) (pabyRec+m_panFieldOffset[iField]), (char *) pValue, j );
    break;
  }
